...
function calendarHeatmapMini() {
    ...
    var startDate = null;
    var data = [];
    //praisebak - title list for adding tooltip title text
    var titleData = [];
    var max = null;
    var colorRange = ['#D8E6E7', '#007950'];
    ...
    // setters and getters
    chart.titleData = function (value) {
        if (!arguments.length) {
            return titleData;
        }
        titleData = value;
    ...
    function chart() {
        ...
        function drawChart() {
            var svg = d3.select(chart.selector())
                ...
                .attr('viewBox', legendEnabled ? '-15 -15 900 160' : '-15 -15 860 135')
            ...
            dayRects.enter()
                ...
                .attr('fill', function (d) {
                    let c = countForDate(d);
                    c = Math.min(c, 4);
                    if (c == 0) {
                        return "rgb(221, 223, 224)";
                    }
                    return color(c - 1);
                    ...
                    if (chart.tooltipEnabled()) {
                        dayRect
                            .on('mouseover', function (d, i) {
                                ...
                                var tooltipLabel = tooltipText(d,idx);
            ...
            if (chart.legendEnabled()) {
                ...
                legendGroup.append('text')
                    ...
                    .attr('x', (width - legendWidth + SQUARE_PADDING) + (colorRange.length + 1) * 13 + 10)
                    .attr('y', height + SQUARE_LENGTH)
                    .text(locale.More);
            }
        ...
        function tooltipText(d,i) {
            var dateStr = moment(d).format('YY/MM/DD');
            var count = countForDate(d);
            let title = titleData[i];
            return (count ? count.toLocaleString() : locale.No) + ' ' + pluralizedTooltipUnit(count) + ' ' + locale.on + ' ' + dateStr + ' ' + title;
        }
